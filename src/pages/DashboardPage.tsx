  ...
  const pazientiMock = [
    {
      ...
      trend: [12, 10, 8, 7, 6],
      aiPreview: ["Leucocitosi marcata", "Possibile processo infiammatorio acuto"],
    },
    {
      id: "2",
      name: "Grace",
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {pazientiMock.map((p) => (
          <div
            key={p.id}
            className="bg-white shadow-sm border rounded-xl p-6 flex flex-col justify-between"
          >
            {/* HEADER */}
            ...
            {/* GRIGLIA FISSA TREND + ANTEPRIMA AI */}
            <div className="grid grid-cols-2 gap-4 mt-4">
              {/* TREND BOX - posizione fissa */}
              ...
              {/* AI PREVIEW BOX - posizione fissa */}
              <div className="border-2 border-sky-200 rounded-lg p-2 bg-white min-h-[110px] flex flex-col">
                <div className="font-semibold text-sm mb-1 text-slate-600">
                  Anteprima AI
                </div>
                ...
                <ul className="text-xs text-slate-700 space-y-1">
                  {p.aiPreview.length > 0 ? (
                    p.aiPreview.map((b, i) => <li key={i}>• {b}</li>)
                  ) : (
